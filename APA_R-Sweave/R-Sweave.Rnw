...
The second equally important benefit of having your manuscript and R code \citep{Rcore} in the same place is that you no longer need to run the analyses and then take the long tedious process of copying and pasting all of the values over to the document (not to mention if something changes and have to redo it\ldots). Using Sweave you can embed the analyses in the document and then have it automatically print values (e.g., means, standard deviations, F-values, p-values, etc) and better yet, if you change any analyses, it will automatically update in your document with the new values! In this document, I will include various common elements of an APA style paper including headings (levels 1:4), lists; \emph{italic} text (also can be done with \code{$\backslash$textit\{\}}); \textbf{bold} face text; inserting R code both as chunks and as single values or lines; effect sizes (e.g., $\eta^2$) , \emph{F}-ratios, and \emph{p}-values (as well as how to use R to insert these values automatically); citing in parentheses and in-text; as well as inserting tables and figures.
\subsection{Affect} % Secondary heading
I study emotion and decision making, and this template will semi-reflect that. In this section I would talk about affect. Researchers use affect as the umbrella term which encompasses both mood and emotion.
...
We had 54 participants (50\% female; see Table~\ref{tab:maintable}). Insert more information about the participants.  % In order to use type a percent sign you have to type \%
...
\includegraphics[width=2in]{Rlogo.png} % Calls in the file and specifies dimensions.
...
if(x > .1) {paste("> ", ".05", sep="")} # If p value is greater than .1 then print "> .05"
if(x > .05 & x <= .1) {paste("= ", round(x,2), sep="")} # If p value is greater than .05 but less than .1 then print "= " + actually p-value rounded to 2 decimal places
if(x > .01 & x <= .05) {paste("= ", round(x,2), sep="")} # If p value is greater than .01 but less than .05 then print "= " + actually p-value rounded to 2 decimal places
# alternatively can be {paste("< ", ".05", sep="")} which prints "< .05"
if(x > .001 & x <= .01) {paste("< ", ".01", sep="")}  # If p value is greater than .001 but less than .01 then print "> .01"
if(x < .001) {paste("< ", ".001", sep="")} # If p value is greater than .001 then print "> .001"
}
